...
SCHEDULER.every '1h', :first_in => '1s' do |job|
	...
	pulls_by_period_repo = backend.pull_count_by_status(
		:period=>'month', 
		:orgas=>(ENV['ORGAS'].split(',') if ENV['ORGAS']), 
		:repos=>(ENV['REPOS'].split(',') if ENV['REPOS']),
		:since=>ENV['SINCE'],
	)
	pulls_by_period_repo.each do | key, pulls_by_period_ungrouped|
		series = [[],[]]
	  pulls_by_period = pulls_by_period_ungrouped.group_by_month(ENV['SINCE'].to_datetime)
